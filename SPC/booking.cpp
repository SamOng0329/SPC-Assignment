		...
		if(choice == 1){
			...
			if(isStaff){
				...
				if(!viewBookingRecord(bookings, id)){
					cout << "  [!] Booking not found. Please try again." << endl << endl;
				}
			}
			else{
...
bool createBooking(vector<Booking> &bookings, vector<Bicycle> &bicycles, vector<Customer> &customers, const string &customerID){
	...
	if(cust == nullptr || !(*cust).isActive){
		cout << "  [!] Customer not found or account inactive." << endl << endl;
		return false;
	}
	...
	do{
		...
		if(bicycle == nullptr){
			cout << "  [!] Bicycle ID not found. Please try again." << endl << endl;
		}
		else if(!(*bicycle).isAvailable){
			cout << "  [!] This bicycle is under maintenance / not in service. Please try again." << endl << endl;
		}
	...
	do {
		...
		if(!isBicycleFreeForSlot(bookings, bicycle->id, slotStart, slotEnd)){
			cout << "  [!] This bicycle is already booked during that time window." << endl;
...
bool viewMyBookings(const vector<Booking> &bookings, const string &customerID){
    ...
    cout << "  +----------+------------------------+--------------+----------+------------+--------------+------------+" << endl;
    cout << "  | ID       | Bicycle                | Date         | Time     | Duration   | Total (RM)   | Status     |" << endl;
    cout << "  +----------+------------------------+--------------+----------+------------+--------------+------------+" << endl;

	// const = read-only reference, auto = auto detect data type
    for(const auto &b : bookings){
		if(isBookingOwnedByCustomer(b, customerID)){
			string bicycleDisplay = b.bicycleName;
			if(bicycleDisplay.length() > 22){
				bicycleDisplay = bicycleDisplay.substr(0, 19) + "... ";
			}

			cout << "  | " << left  << setw(8)  << b.bookingID
                 << " | " << left  << setw(22) << bicycleDisplay
                 << " | " << left  << setw(12) << b.startDate
                 << " | " << left  << setw(8)  << b.startTime
                 << " | " << right << setw(8)  << b.durationHours << "h "
                 << " | " << right << setw(12) << b.totalAmount
...
bool modifyBooking(vector<Booking> &bookings, vector<Bicycle> &bicycles, const string &bookingID, const string &requesterID, bool isStaff){
	...
	do{
		...
		cin >> hours;
		if(!isDurationValid(hours)){
			cout << "  [!] Invalid duration. Must be between " << MIN_DURATION_HOURS << " and " << MAX_DURATION_HOURS << " hours" << endl << endl;
		}
	} while(!isDurationValid(hours));
	...
	do{
		...
		if(!toTotalMinutes(newDate, newTime, slotStart)){
			cout << "  [!] Invalid date or time. Please check the format and values." << endl << endl;
			continue;
		}
...
bool cancelBooking(vector<Booking> &bookings, const string &bookingID, const string &requesterID, bool isStaff){
	...
	cout << "  [DONE] Booking " << bookingID << " has been cancelled." << endl << endl;
	return true;
}

void viewFullSchedule(const vector<Booking> &bookings){
	if(bookings.empty()){
		cout << "  [!] No bookings found." << endl << endl;
		return;
	}
	...
	for (const auto& b : bookings) {
        ...
        cout << "  | " << left  << setw(8)  << b.bookingID
             ...
             << " | " << right << setw(6)  << b.durationHours << "h "
             << " | RM" << right << setw(8)  << b.subtotal
             << " | RM" << right << setw(7)  << b.tax
             << " | RM" << right << setw(8)  << b.totalAmount
             << " | " << left  << setw(6)  << b.status << " |" << endl;
    }
...
string generateBookingID(const vector<Booking> &bookings){
	...
	oss << "BK" << setw(3) << setfill('0') << nextNum;
	return oss.str();
}
